...
// Виджет записи GetCourse. ID унаследован с первой версии ленда — чужой, менять перед запуском.
...
export function GetCourseWidget() {
  ...
  useEffect(() => {
    ...
    script.async = true
    script.onload = () => {
      // DOMContentLoaded в SPA уже отработал — автозапуск виджета не сработает, дёргаем вручную.
      try {
  ...
  return <div ref={ref} className="getcourse-widget w-full" />
}
